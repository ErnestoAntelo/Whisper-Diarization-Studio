...
- Abre `start.bat` y pulsa **Añadir archivo…** para seleccionar audio o vídeo de cualquier carpeta (MP3, WAV, M4A, FLAC, AAC, MP4, MKV, MOV, WebM, AVI y otros). El filtro **Todos los archivos (FFmpeg)** permite elegir formatos adicionales; deben contener una pista de audio decodificable. Quedan marcados automáticamente y no necesitas copiarlos a `input`.
...
- **Abrir editor…** muestra **Mis transcripciones**: una entrada por trabajo, con búsqueda por nombre. Los TXT, informes de diarización y copias internas no aparecen como alternativas. Cada audio de la lista también permite abrir directamente su editor. La app busca el audio original; si falta, permite leer y corregir el texto sin reproducción. El menú **•••** permite abrir la carpeta de resultados.
...
- El editor agrupa las frases consecutivas del mismo hablante y separa las pausas de más de tres segundos. El selector **Por hablante / Frase a frase** indica el modo activo: por defecto muestra párrafos con un intervalo de tiempo por bloque. **Frase a frase** separa todo; **Editar frases** separa solo un bloque para corregirlo. **Por hablante** vuelve a agrupar. El JSON conserva las frases y sus tiempos originales. **Guardar** actualiza únicamente el proyecto editable; las copias exportadas se generan con **Exportar…**.
...
- **Exportar…** en el editor permite elegir TXT, SRT, VTT, CSV o JSON, nombre y destino. TXT usa la vista actual y permite quitar tiempos y hablantes; SRT/VTT conservan los tiempos por frase; CSV y JSON conservan todas las frases. Por defecto se propone `exports` dentro del trabajo. **Abrir proyecto externo…** en la biblioteca abre las copias JSON.
- **Ctrl+Mayús+S** abre Exportar. **Ctrl+E** abre el editor. **Ctrl+O** abre el selector de audios y **Ctrl+S** guarda desde el editor. Al iniciar un trabajo se recuerdan la última carpeta de audio y el destino de resultados.
...
4.  Click **Añadir archivo…** to select files, or **Añadir carpeta…** to add a folder. Added files are selected automatically.
...
*   Ideas: improve subtitle line breaking and add more export formats.
...
Cada trabajo conserva su JSON editable, el TXT inicial y, si hay diarización, un informe técnico. Los trabajos nuevos guardan también la ruta del medio original en `.source.json`; los exportados van por defecto en `exports/`. La biblioteca muestra los trabajos, sin los informes ni las exportaciones. No se han borrado ni movido archivos anteriores. Guardar en el editor no actualiza copias TXT antiguas: usa Exportar para obtener una copia con las últimas correcciones.
